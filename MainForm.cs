...
public sealed class MainForm : Form
{
    ...
    private Control BuildSidebar()
    {
        ...
        footer.Controls.Add(LabelOf("Developed by drJhonatan00", 0, 4, 180, 18, 8, FontStyle.Bold, TextSecondary));
    ...
    private Control BuildDashboard()
    {
        ...
        panel.Controls.Add(LabelOf("BACKUP SETUP", 0, 0, 200, 22, 8, FontStyle.Bold, TextSecondary), 0, 2);
        panel.Controls.Add(BuildSetupCard(), 0, 3);
        panel.Controls.Add(LabelOf("ACTIVITY", 0, 0, 200, 22, 8, FontStyle.Bold, TextSecondary), 0, 4);
    ...
    private Control BuildSetupCard()
    {
        ...
        fileList.Location = new Point(22, 96); fileList.Size = new Size(390, 205); fileList.BorderStyle = BorderStyle.FixedSingle; fileList.CheckOnClick = true; fileList.Font = new Font("Segoe UI", 9); fileList.IntegralHeight = false; fileList.BackColor = SurfaceAlt; fileList.ForeColor = TextPrimary; fileList.Anchor = AnchorStyles.Top | AnchorStyles.Bottom | AnchorStyles.Left; fileList.SelectedIndexChanged += (_, _) => UpdateStats(); card.Controls.Add(fileList);
        card.Controls.Add(LabelOf("DESTINATION FOLDER", 440, 18, 300, 18, 8, FontStyle.Bold, TextSecondary));
        destinationBox.Location = new Point(440, 42); destinationBox.Size = new Size(425, 31); destinationBox.Anchor = AnchorStyles.Top | AnchorStyles.Left | AnchorStyles.Right; destinationBox.BorderStyle = BorderStyle.FixedSingle; destinationBox.Font = new Font("Segoe UI", 9.5F); card.Controls.Add(destinationBox);
    ...
    private void SetStatus(string text, bool success) { statusLabel.Text = text; statusLabel.ForeColor = success ? Color.FromArgb(30, 166, 114) : Color.FromArgb(214, 78, 78); }
    private void Log(string text) { activityLog.AppendText($"[{DateTime.Now:HH:mm:ss}]  {text}{Environment.NewLine}"); activityLog.ScrollToCaret(); }
    ...
    private void StyleButton(Button button, Color back, Color fore) { button.FlatStyle = FlatStyle.Flat; button.FlatAppearance.BorderSize = 0; button.BackColor = back; button.ForeColor = fore; button.Font = new Font("Segoe UI", 9, FontStyle.Bold); button.Cursor = Cursors.Hand; }
}
